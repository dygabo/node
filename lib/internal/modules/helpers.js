...
function addHooks(hooks) {
  // TODO(joyeecheung): figure out how people customize the resolution.
  // via monkey-patching and provide a proper alternative.
  if (hooks.resolve) {
  ...
  if (hooks.export_hook) {
    validateFunction(hooks.export_hook, 'hooks.export_hook');
  }
  ArrayPrototypePush(synchronousHooks, hooks);
}
