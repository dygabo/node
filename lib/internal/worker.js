...
const kIsHooksThread = Symbol('kIsHooksThread');
...
class Worker extends EventEmitter {
  constructor(filename, options = kEmptyObject) {
    ...
    super();
    const isHooksThread = this.#isHooksThread = arguments[2] === kIsHooksThread;
    debug(
      `[${threadId}] create new worker`,
      filename,
      options,
      `isHooksThread: ${isHooksThread}`,
    );
    if (options.execArgv)
    ...
    this[kPort].postMessage({
      ...
      filename,
      doEval,
      isInternal: isHooksThread,
      cwdCounter: cwdCounter || workerIo.sharedCwdCounter,
      workerData: options.workerData,
      environmentData,
      publicPort: port2,
      hooksPort: !isHooksThread ? toHooksThread : undefined,
    ...
    if (!isHooksThread && hasCustomizations) {
  ...
  [kOnExit](code, customErr, customErrReason) {
    ...
    if (!this.#isHooksThread && hasCustomizations) {
...
class HooksWorker extends Worker {
  constructor(filename, options) {
    super(filename, options, kIsHooksThread);
  }
